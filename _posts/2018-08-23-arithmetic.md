...
数组和链表在功能上面来说可以代替栈，但是，**数组和链表暴露太多接口**，操作上的灵活会导致使用的不可控。用数组实现称为**顺序栈**，用链表实现称为链式栈
多用于**先进后出**，对称匹配 场景。 
...
- 堆以数组结构来存放，左节点为2i，右节点为2i+1
- 堆化：堆中元素插入和删除后，需要进行调整，让其满足堆的特性，这个过程叫做堆化（从下往上，或从上往下进行元素删除，增加，每个节点进行对比，交换位置）。
	- 增加节点，将元素放到末尾节点，不断和父节点进行比较，如果大于（小于）父节点则相互交换位置，交换i, i/2两个节点位置
	- 删除节点，从末尾节点与删除节点交换位置（避免堆化完后数据空洞），然后自上而下不断比较交换节点
- 应用
    - 优先级队列
    - 合并有序小文件（入堆，出堆）
    - 高性能定时器（始终拿堆顶数据）
