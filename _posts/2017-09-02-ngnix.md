...
### **业务开发与nginx问题排查**
问题排查神器：日志与状态码
线上出了问题，第一步永远是看日志，第二步是看状态码。
① 找准两大日志
Nginx 默认有两类日志，排查问题时先看它们在哪：

access.log（访问日志）：记录每一次请求。用来分析接口响应时间（$request_time 和 $upstream_response_time）、QPS、客户端 IP。

error.log（错误日志）：记录 Nginx 自身的错误或转发失败。排查接口 5xx 错误的首要工具。

403 Forbidden	静态资源权限不足，或 Nginx 拦截规则（如 IP 黑名单、安全防护模块触发）。	检查 Nginx 运行用户是否有对应目录的读权限；检查 WAF 规则。
404 Not Found	路由配错了，或者静态资源路径（root / alias）写错。	确认 location 匹配规则与后端实际接口路径是否一致。
500 Internal Server Error	极少是 Nginx 自身问题。多为后端服务挂了，或者 Nginx 配置了无法解析的变量。	看后端应用日志（如 Spring Boot / Go 报错）。
502 Bad Gateway	后端应用彻底挂了（如进程挂掉、端口没启动），或者 Nginx 与后端连接被拒绝。	检查后端服务进程是否存在，端口是否正常监听。
504 Gateway Timeout	后端处理太慢，超出了 Nginx 的等待时间。	优化后端慢 SQL/慢接口；或者在 Nginx 中调大

1 客户端真实 IP 丢失
当请求经过 Nginx 反向代理后，后端应用（如 Java 中的 request.getRemoteAddr()）拿到的往往是 Nginx 服务器的内网 IP。
为了获取用户真实 IP，必须在 Nginx 中传递 Header：
