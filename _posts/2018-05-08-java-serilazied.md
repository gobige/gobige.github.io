...
### Dubbo 和springcloud 接口返回体也需要实现serialized接口吗
dubbo： 
- 默认序列化协议：Dubbo 默认使用 Hessian2 作为序列化框架（也可以配置成 Kryo、FST、Protobuf 等）。即使 Hessian2、Kryo 等不是 Java 原生的序列化，但为了兼容 Java 生态和安全规范，Hessian2 在底层进行对象序列化时，依然会严格校验该对象是否实现了 java.io.Serializable 接口。

springcloud：
- Spring Cloud 的微服务之间通信，传输的是 HTTP 协议的文本数据（通常是 JSON 字符串）。当你用 Feign 调用另一个服务时，Spring 默认使用 Jackson（或 Gson）把你的 Java 对象转成一串 JSON 字符串（也就是 {"id": 1, "name": "Mr.Chen"}），然后通过 HTTP 协议发过去。对方收到后，再用 Jackson 把 JSON 字符串还原成 Java 对象
