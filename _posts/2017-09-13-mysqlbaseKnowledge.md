...
- Soundex 返回串的soundex值
  - 完全不支持中文拼音：SOUNDEX 纯粹是为英文发音设计的。如果你输入中文（如 SOUNDEX('张三')），它无法进行有效的拼音声母韵母分析。
  - 首字母敏感：因为算法强行保留首字母，如果首字母拼错了，整个特征码就会完全不同。例如 Pfizer（辉瑞，P不发音）和 Fizer 读音一样，但由于首字母不同，它们计算出的特征码完全对不上。
  - 索引失效问题：如果在普通的 name 字段上使用 WHERE SOUNDEX(name) = ...，会导致全表扫描，无法走 name 字段的索引。

解决方案：如果业务中非常依赖语音匹配，可以在表中单独建一个 name_soundex 字段，并在写入数据时提前计算好存进去，给该字段建索引。
...
Mysqldump缺点：
1. 备份和恢复速度慢（致命痛点 ⚠️）
- 备份慢：它是单线程运行的。它需要一行一行地将数据从磁盘读出，再拼装成 INSERT 语句写入文件。当数据量达到数十 GB 甚至 TB 级别时，备份会非常耗时。
- 恢复更慢：恢复时，MySQL 需要逐行执行 SQL 语句。这意味着它要重新经历：SQL 解析 -> 构建 B+ 树索引 -> 写入数据 -> 写 Redo/Undo Log 的完整过程。由于涉及大量的磁盘 I/O 和索引重建，数据量大时恢复可能需要几个甚至十几个小时。

2. 备份期间对服务器性能（CPU/IO）消耗大
- 在备份过程中，mysqldump 会进行全表扫描。这会将大量冷数据加载到 InnoDB 的 Buffer Pool 中，从而把热数据“挤”出去，导致数据库的缓存污染。
- 备份期间由于大量的数据读取和写入，会导致服务器 I/O 飙升，直接影响线上业务的响应速度。

3. 一致性备份需要锁表（对非 InnoDB 引擎不友好）
- 为了保证备份数据的一致性（即备份出来的数据是同一个时间点的快照）：
- 对于 InnoDB 引擎：可以配合 --single-transaction 参数，利用 MVCC（多版本并发控制）实现无锁一致性备份，不影响业务写入。
- 对于 MyISAM 引擎或含有 DDL 操作时：必须使用 --lock-tables 或 --master-data 进行锁表（全局读锁 FTWRL），此时整个数据库会变成只读状态，线上写业务会被直接卡死。
