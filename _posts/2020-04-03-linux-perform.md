...
**注意**：1避免性能测试工具干扰程序性能 2避免外部环境变化  3要忍住“把 CPU 性能优化到极致”的冲动
...
pcstat：基于Go语言开发，查看文件在内存中的缓存大小及缓存比例
...
处在文件系统和磁盘驱动中间的一个块设备抽象层。1向上，为文件系统和应用程序，提供访问块设备的标准接口；向下，把各种异构的磁盘设备抽象为统一的块设备，并提供统一框架来管理这些设备的驱动程序
2文件系统和应用程序发来的 I/O 请求排队，并通过重新排序、请求合并等方式，提高磁盘读写的效率
...
3. 网络包送到网络接口层，进行物理地址寻址，找到下一跳MAC地址。添加帧头和帧尾，放到发包队列。然后软中断通知驱动程序
...
**C10K**问题 跳过内核协议栈的冗长路径，把**网络包直接送到应用程序**
