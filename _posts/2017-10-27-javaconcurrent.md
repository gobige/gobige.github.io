...
- runnable->blocked：线程等待synchronized隐式锁。JVM层面不关系操作系统调度状态，等待CPU和等待I/O都是runnable状态
	- 实际场景：并发争抢同一把锁（synchronized）。 两个高并发请求同时尝试修改同一个借款人的账户余额，在数据库或 Java 语言层面使用 synchronized 关键字控制并发，没抢到锁的那个线程就会被强行阻塞（BLOCKED）。 
- runnable->waiting：1获得synchronized锁线程，调用无参Object.wait() 2调用无参Thread.join方法 3调用LockSupport.park方法
	- 实际场景：生产者-消费者模型（线程池底层核心）。
		- 在线程池中，当核心线程（Core Thread）做完了手头的所有工作后，它会去去任务队列（LinkedBlockingQueue）里取新任务。如果队列是空的（没活干了），核心线程就会调用 Condition.await() 或 LockSupport.park() 进入 WAITING 状态常驻内存，处于“原地待命”状态，不消耗 CPU。一旦有新任务进来，才会被重新唤醒。
- runnable->timed_waiting：
	- 实际场景：
		- 定时任务/重试机制：调用第三方支付接口失败，代码里写了 Thread.sleep(2000) 等待 2 秒后发起下一次重试。
		- 超时控制：从微服务的数据库连接池（Druid/HikariCP）里获取连接，由于连接满了，线程进入等待，最多等 30 秒（maxWait），超时就报错。
...
- runnable->terminated：线程执行完run方法
- waiting->runable：
	- 注意一个高频面试/排查深坑：处于 WAITING 状态的线程被 notify() 叫醒后，它不是直接变成 RUNNABLE。因为叫醒它之后，它还要去重新争抢锁。如果此时锁被别人占着，它会先从 WAITING 状态变成 BLOCKED 状态，抢到锁之后才会真正变成 RUNNABLE。

理解Java线程各种状态和生命周期对于诊断多线程Bug很有帮助，分析线程dump，stack信息，查看线程状态，调用栈和锁信息。

- 如果发现大量线程处于 BLOCKED 状态：说明代码里有严重的锁竞争（死锁、或者 synchronized 块里写了耗时的远程 RPC 调用）。
- 如果发现大量线程处于 TIMED_WAITING 状态，且卡在网络 I/O 处：说明后端调用第三方接口（如银行网关）时没有设置合理的 ReadTimeout，或者对方响应极慢，导致你的线程全部在原地干等。
- 如果发现大量线程处于 WAITING 状态：通常是正常的线程池空闲状态，如果数量异常庞大，需要检查线程池的最大线程数和核心线程数配置是否合理，防止内存溢出（OOM）。
...
Java内存模型时很复杂的规范，规范了JVM如何提供**按需禁用缓存和编译优化**的方法。包括volatile，synchronized和final关键字，以及六项Happends-Before规则
...
多线程解决了硬件利用率的问题，主要是CPU和I/O设备综合利用率问题
...
### 三大性质
- 原子性 一个或多个操作不可中断，要么全部成功，要么全部失败；主要由线程切换导致
- 有序性 操作执行顺序不可变；主要由编译优化导致
- 可见性 可见性是指当一个线程修改了共享变量后，其他线程能够立即得知这个修改；主要由于缓存导致
...
**为什么 synchronized 一开始是偏向锁？**
- 在早期的 Java 版本中，synchronized 只要一加锁就会直接向操作系统申请内核互斥量，也就是直接升级为 重量级锁。这涉及到用户态到内核态的上下文切换，性能极差。为了优化，Sun/Oracle 的工程师对大量线上真实运行的 Java 程序进行了大数据分析，得出了一个惊人的统计结论：在绝大多数情况下，锁不仅不存在多线程竞争，而且总是由同一个线程多次获得。
	- 概率极大的场景（早期的基础类库设计）：Java 语言拥有非常重的历史包袱。在 Java 早期，许多官方提供的核心基础工具类都是线程安全的，内部堆满了 synchronized：StringBuffer / Vector / Hashtable；在实际业务中，我们经常会在一个方法（单线程）内局部拼装字符串。这时候，同一个线程会连续、重复地去拿 StringBuffer 内部的这把锁

**JDK 18 之后，偏向锁在 HotSpot 源码中被正式移除**
- 在当今的互联网后端业务中，“同一线程重复拿锁”的场景正在被稀释：
	- 代码规范的演进：我们现在几乎不再使用 Vector、Hashtable 或 StringBuffer，而是首选无锁的 ArrayList、HashMap、StringBuilder。
	- 线程池的广泛应用：在微服务（如 Spring Boot、Tomcat）环境中，请求是由线程池异步处理的。

受保护资源和锁之间合理关联关系应该是N:1关系
...
2. 请求与保持条件 线程T已得到共享资源X，在等待共享资源Y时，不释放共享资源X
3. 不剥夺条件 其他线程不可占用线程T1占有资源
4. 循环等待条件 线程T1等待线程T2占有资源 线程T2等待线程T1资源 
...
作为业务开发人员，在代码层面预防线程饥饿，可以遵循以下原则：
- 控制锁的持有时间：synchronized 块或 lock() 之间只写核心临界区代码，绝对不要在持锁期间进行耗时的 RPC 远程调用或大文件 I/O 操作。
- 合理选择锁类型：高并发且对顺序有严格要求的金融/排队场景，使用 new ReentrantLock(true) 开启公平锁；读多写极多的场景，首选 StampedLock。
- 线程池隔离：核心业务与非核心业务、父任务与子任务必须做物理上的线程池隔离，防止低优先级任务反向吞噬核心资源。
...
synchronized关键字及wait()、notify()、notifyAll()这三个方法都是管程的组成部分；管程和信号量是等价的，管程能够实现信号量，信号量也能实现管程

管程解决了并发编程中**互斥**（对共享变量及共享变量操作统一封装），**同步**（通过条件变量和等待队列）问题

Java参考MESA模型，语言内置管程对MESA模型进行精简。MESA模型中，条件变量有多个，Java内置管程（synchronize）只有一个条件变量。
...
- volatile：只能保证 **可见性 和 有序性**，不能保证原子性。
	- 可见性：当一个线程修改了 volatile 修饰的变量，新值会立刻被刷新回主内存，其他线程读取时会直接从主内存读取最新值，避免读到“过期数据”。	
	...
	- 有序性：通过在底层插入内存屏障（Memory Barrier），禁止 JVM 和 CPU 对代码进行指令重排优化。
- synchronized：全能型选手。同时保证 **原子性、可见性 和 有序性**。
	- 原子性：它是一把独占锁（排他锁）。被它包裹的代码块在同一时刻只能有一个线程进入执行，整个执行过程是不可分割的整体，完美解决多线程并发冲突。
2. 修饰的范围不同
- volatile：是变量修饰符，只能用来修饰**成员变量（静态变量或实例变量）**。
- synchronized：是**代码块或方法**修饰符，可以修饰实例方法、静态方法，或者指定锁对象修饰代码块。
3. 性能与开销不同（重量级差异）
- volatile：是轻量级的同步机制。它不会引起**线程的阻塞和上下文切换**，其执行开销极低，几乎等同于普通变量的读写。
- synchronized：是重量级的同步机制（尽管有偏向锁、轻量级锁等优化）。未抢到锁的线程会进入 BLOCKED 状态，存在线程挂起和恢复的上下文切换，开销相对较大。

**volatile 的适用场景**
- 由于不带锁，它最适合“一写多读”或者只做状态标记的场景：
	- 场景 A：独立的状态标志位（控制线程开关）
 	- 场景 B：单例模式的 DCL（双重检查锁）
  
**synchronized 的适用场景**
- 适用于“多写多读”、涉及复杂的业务逻辑复合操作（如：先查、再账务计算、最后修改）的场景：
	- 资金扣减、商品减库存。
	- 线程池内部转储任务队列。
	- 多个线程需要保证严格的先后执行顺序或互斥访问。
...
无状态对象内部没有属性，只有方法；无状态服务，协议；没有线程安全问题
...
Loom项目 轻量级线程Fiber 轻量级的线程，创建的成本很低，基本上和创建一个普通对象的成本相似  ，
协程
...
1. CountDownLatch（倒计时器）—— 减法计数
- 设计思想：一个或多个线程，等待其他多个线程完成某件事情之后才能继续执行。类似于接力赛或者火箭发射倒计时。
- 核心机制：主线程（等待线程）调用 await() 方法。如果计数器没有归零，主线程会一直卡在 await() 处阻塞排队。一旦计数器减到 0，主线程瞬间被唤醒激活，继续往下执行。
- 宿命：一次性的。一旦计数器变成 0，就再也无法重置或复用了。
- 场景 A：并行网关聚合数据（大批量异步拼装） 当用户访问“个人中心”时，后端需要同时调用：① 核心基本信息服务、② 账户余额服务、③ 近期订单服务
- 场景 B：并发测试发令枪（多等一）
2. CyclicBarrier（循环栅栏）—— 加法计数/人齐发车
- 设计思想：一组线程互相等待，直到所有线程都到达某一个同步点（栅栏）时，这组线程再同时继续往下执行。类似于大巴车司机：“大家都到齐了（都坐到座位上），我才发车”。
- 核心机制：每个工作线程在跑完自己的阶段性任务后，调用 await() 方法。此时，当前工作线程会立刻在 await() 处被阻塞挂起，等待其他兄弟线程。当最后一个（第 4 个）线程也调用了 await() 时，说明全员到齐，屏障开启，所有被阻塞的线程同时被唤醒，继续并发往前冲。
- 特性：可循环复用（Cyclic）。当所有人冲过栅栏后，计数器会自动重置，可以立即投入下一轮的协同等待中。CyclicBarrier提供方法更多，可在多个线程某个事件达成的时候执行barrierAction事件
