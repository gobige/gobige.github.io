...
- **快照读（普通 select）**：基于 MVCC 实现，事务内使用同一个一致性视图，其他事务插入的新数据不可见，不会出现幻读；
- **当前读（select ... for update、insert/update/delete）**：通过 Next-Key Lock（记录锁 + 间隙锁）锁定查询范围的间隙，防止其他事务插入新数据，避免幻读。
